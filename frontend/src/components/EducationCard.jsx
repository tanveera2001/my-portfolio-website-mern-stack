    ...
    <div className="relative bg-white rounded-2xl shadow p-4 sm:p-6 w-full">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-6">
        <div className="flex-1 min-w-0 break-words">
          ...
          {degree && <h3 className="text-base sm:text-lg font-semibold break-words">{degree}</h3>}
          {university && <p className="text-sm sm:text-base text-gray-700 break-words">{university}</p>}
        ...
        {logo && (
          <div className="flex-shrink-0 w-full sm:w-auto mt-3 sm:mt-0 flex justify-center sm:justify-end">
            <img src={logo} alt={university || "logo"} className="h-20 sm:h-24 w-auto object-contain" />
          </div>
        )}
      </div>
    </div>
